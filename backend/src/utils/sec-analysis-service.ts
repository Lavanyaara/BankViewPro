...
export class SECAnalysisService {
  ...
  extractMetrics(analysisText: string): ExtractedMetrics {
    ...
    const roeMatch = analysisText.match(/ROE[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (roeMatch) metrics.roe = parseFloat(roeMatch[1]);

    // Extract ROA (Return on Assets)
    const roaMatch = analysisText.match(/ROA[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (roaMatch) metrics.roa = parseFloat(roaMatch[1]);

    // Extract NIM (Net Interest Margin)
    const nimMatch = analysisText.match(/NIM[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (nimMatch) metrics.nim = parseFloat(nimMatch[1]);

    // Extract CET1 Ratio
    const cet1Match = analysisText.match(/CET1[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (cet1Match) metrics.cet1Ratio = parseFloat(cet1Match[1]);

    // Extract Tier 1 Ratio
    const tier1Match = analysisText.match(/Tier\s*1[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (tier1Match) metrics.tier1Ratio = parseFloat(tier1Match[1]);

    // Extract Leverage Ratio
    const leverageMatch = analysisText.match(/leverage\s*ratio[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (leverageMatch) metrics.leverageRatio = parseFloat(leverageMatch[1]);

    // Extract LCR (Liquidity Coverage Ratio)
    const lcrMatch = analysisText.match(/LCR[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (lcrMatch) metrics.lcr = parseFloat(lcrMatch[1]);

    // Extract NSFR (Net Stable Funding Ratio)
    const nsfrMatch = analysisText.match(/NSFR[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (nsfrMatch) metrics.nsfr = parseFloat(nsfrMatch[1]);

    // Extract NPL Ratio
    const nplMatch = analysisText.match(/NPL\s*ratio[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (nplMatch) metrics.nplRatio = parseFloat(nplMatch[1]);

    // Extract Charge-off Rate
    const chargeOffMatch = analysisText.match(/charge-?off\s*rate[:\s]*(?:≈\s*)?(\d+\.?\d*)%/i);
    if (chargeOffMatch) metrics.chargeOffRate = parseFloat(chargeOffMatch[1]);
