...
def render_capitalization(bank_data, institution_name):
    ...
    st.subheader("📈 Five-Year Capital Trends")
    
    # Create trend charts for each metric group
    col1, col2 = st.columns(2)
    
    with col1:
        # Capital ratios chart
        ratio_metrics = ['capital_adequacy_ratio', 'tier1_ratio', 'leverage_ratio']
        fig_ratios = create_trend_chart(
            historical_data,
            ratio_metrics,
    ...
    with col2:
        ...
        fig_rwa = go.Figure()
        fig_rwa.add_trace(go.Scatter(
            x=historical_data['year'],
            y=historical_data['risk_weighted_assets'],
            mode='lines+markers',
            name='Risk Weighted Assets',
            line=dict(color='#d62728', width=3),
            marker=dict(size=8),
            fill='tonexty' if len(historical_data) > 1 else None
        ))
        
        fig_rwa.update_layout(
            title="Risk Weighted Assets Trend",
            xaxis_title="Year",
    ...
    # Gauge Charts for Current Performance
    st.subheader("🎯 Current Performance vs Benchmarks")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        # Capital Adequacy Ratio Gauge
        car_info = metric_info['capital_adequacy_ratio']
        fig_car_gauge = create_metric_gauge(
            latest_data['capital_adequacy_ratio'],
            car_info['benchmark']['good'],
            car_info['benchmark']['fair'],
            "Capital Adequacy Ratio",
            "%"
        )
        st.plotly_chart(fig_car_gauge, use_container_width=True)
    
    with col2:
        # Tier 1 Ratio Gauge
        tier1_info = metric_info['tier1_ratio']
        fig_tier1_gauge = create_metric_gauge(
            latest_data['tier1_ratio'],
            tier1_info['benchmark']['good'],
            tier1_info['benchmark']['fair'],
            "Tier 1 Capital Ratio",
            "%"
        )
        st.plotly_chart(fig_tier1_gauge, use_container_width=True)
    
    with col3:
        # Leverage Ratio Gauge
        leverage_info = metric_info['leverage_ratio']
        fig_leverage_gauge = create_metric_gauge(
            latest_data['leverage_ratio'],
            leverage_info['benchmark']['good'],
            leverage_info['benchmark']['fair'],
            "Leverage Ratio",
            "%"
        )
        st.plotly_chart(fig_leverage_gauge, use_container_width=True)
